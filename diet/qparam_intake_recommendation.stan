...
model {
  ...
  // Prior limits for recommended nutrients
  ...
  // Posterior estimations for the queried variables Q

  Q[1] ~ lognormal(Q_mu[1], Q_sd[1]);
  Q[2] ~ lognormal(Q_mu[2], Q_sd[2]);
  //Q[3] ~ lognormal(Q_mu[3], Q_sd[3]);

  // Concentration estimations whose limits direct the Q estimations

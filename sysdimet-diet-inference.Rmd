...
#for (person_id in 1:length(kontrolli)) {
for (person_id in 29:29) {
  
#failed:
#[1] "Simulating patient id 26"
#[1] "Simulating subject_code S12"

#[1] "Simulating patient id 28"
...
#recommendation_with_concpreds.df$subject_code
...
mean_contributions <- mean_contributions %>%  left_join(assumedpredictors, by = join_by(nutrient == Name)) 
...
cumulative_concs <- df %>% 
    ...
    mutate(predicted_concentration = cumsum(contributions),.groups=person_id) %>%
...
limit_data <- data.frame(
  concentration = assumedtargets$Name,
...
# Include personal effect clustering
personal_effect_clusters$person_id <- factor(personal_effect_clusters$person_id)
personal_effect_clusters$effect_cluster <- factor(personal_effect_clusters$effect_cluster)

cumulative_concs <- cumulative_concs %>% left_join(personal_effect_clusters, join_by(person_id))
...
p <- ggplot(cumulative_concs) +
  geom_point(aes(x = predicted_concentration, y = nutrient, color = effect_cluster), size = 1.0, show.legend = TRUE) +
  geom_path(aes(x = predicted_concentration, y = nutrient, group = person_id, color = effect_cluster), linewidth=0.3, show.legend = FALSE) +

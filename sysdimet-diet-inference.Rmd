...
for (person_id in 1:length(kontrolli)) {
  ...
  if (check_failed_concs(graph_person_id, personal_info, recommeded_concentrations) == TRUE) { 
      ...
      # Save the results
      saveRDS(effects.df, paste0("patient_summary/sysdimet/gamma_qr_mv/simulation", simulation, "/details/effects_", graph_person_id, ".rds"))
...
for (subject_code in recommendation_with_concpreds.df$subject_code) {
  ...
  for (i in 1:length(queried_nodes))
  {
    ...
    {
      effects.df <- readRDS(paste0("patient_summary/sysdimet/gamma_qr_mv/simulation",simulation,"/details/effects_",graph_person_id,".rds"))

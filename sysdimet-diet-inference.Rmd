...
ggplot(as.data.frame(result)) +
  geom_point(aes(y=in_range*preference, x=in_concentration_range, color=lp__), show.legend=FALSE) +
  scale_color_gradientn(colours = colorspace::diverge_hcl(7))

mean(intake_proposals.df$dvit)  

max(result$lp) # -73... -33 (normal l3)
mean(result$l3)
max(result$in_concentration_range) # -23, -35

plot(result$softbound_sum, result$in_range)

plot(result$in_concentration_range, result$in_range)
plot(result$softbound_sum, result$in_range)

hist(result$in_range)

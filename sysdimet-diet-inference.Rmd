...
c(datadesc[datadesc$Condition >= 100,]$Lowerbound, datadesc[datadesc$Condition >= 100,]$Upperbound)
...
for (person_id in 1:1) {
  ...
  V(personal_graph)["fshdl"]$type <- 200
  V(personal_graph)["fsldl"]$type <- 200
  V(personal_graph)["fsins"]$type <- 300
  V(personal_graph)["fskol"]$type <- 300
  V(personal_graph)["fpgluk"]$type <- 300
  ...
  concs.df <- data.frame(proposal=rep(proposals, each = length(proposal_samples)), fshdl=as.vector(t(fshdl)), fsldl=as.vector(t(fsldl)))
  ...
  summarise(
    ...
    fshdl_prob = (ecdf(fshdl)(fshdl_recommendations[2]) - ecdf(fshdl)(fshdl_recommendations[1])),
    fsldl_prob = (ecdf(fsldl)(fsldl_recommendations[2]) - ecdf(fsldl)(fsldl_recommendations[1])),
    lowest_conc_prob = min(fshdl_prob, fsldl_prob),
    iid_prob = fshdl_prob * fsldl_prob,
  )
...
intake_proposals.df[intake_proposals.df$boundsum==4,]
...
```{r fig.height=8, inference_debug}
  ...
  concs.df$lp <- result$lp__
  concs.df$bs <- result$softbound_sum
  concs.df$preference <- result$preference
  
  concs.df <- concs.df[concs.df$bs==4,]
  
    conc_plot1 <- ggplot(concs.df) +
      geom_point(aes(y=lp, x=fshdl, color=bs), show.legend=TRUE) +
    ...
    conc_plot2 <- ggplot(concs.df) +
      geom_point(aes(y=lp, x=fsldl, color=bs), show.legend=FALSE) +
      geom_vline(xintercept = fsldl_recommendations, linetype="solid", color = "black", linewidth=0.4) +
      scale_color_gradientn(colours = colorspace::diverge_hcl(7))
    
    conc_plot3 <- ggplot(concs.df) +
      geom_point(aes(y=lp, x=fsins, color=bs), show.legend=FALSE) +
      geom_vline(xintercept = fsins_recommendations, linetype="solid", color = "black", linewidth=0.4) +
      scale_color_gradientn(colours = colorspace::diverge_hcl(7))

    conc_plot4 <- ggplot(concs.df) +
      geom_point(aes(y=lp, x=fskol, color=bs), show.legend=FALSE) +
      geom_vline(xintercept = fskol_recommendations, linetype="solid", color = "black", linewidth=0.4) +
      scale_color_gradientn(colours = colorspace::diverge_hcl(7))

    conc_plot5 <- ggplot(concs.df) +
      geom_point(aes(y=lp, x=fpgluk, color=bs), show.legend=FALSE) +
    ...
    lp_plot <- grid.arrange(conc_plot1, conc_plot2, layout_matrix = lay, padding=0)

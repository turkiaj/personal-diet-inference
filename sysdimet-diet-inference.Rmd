...
ggplot(as.data.frame(intake_proposals.df)) +
  geom_point(aes(y=preference, x=in_concentration_range, color=lp), show.legend=FALSE) +
  scale_color_gradientn(colours = colorspace::diverge_hcl(7))

ggplot(as.data.frame(result)) +
  geom_point(aes(y=in_range*preference_lpdf, x=(1-in_range)*in_concentration_range_lpdf, color=dvit), show.legend=FALSE) +
  scale_color_gradientn(colours = colorspace::diverge_hcl(7))
...
plot(result$in_range*result$preference_lpdf, result$dvit)
plot(result$dvit, result$lp__)

#plot(result$preference, result$preference_error_sum)

#####

x <- result$softbound_sum

#calculate normal CDF probabilities
prob <- pnorm(x, mean = 10, sd = 5, lower.tail = TRUE)
prob <- pnorm(x, mean = 10, sd = 1)
x <- seq(-10,1,0.01)

x <- result$softbound_sum - 9

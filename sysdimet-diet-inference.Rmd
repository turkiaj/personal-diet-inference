...
max_error <- 30  # includes penalty l5
preference_strength <- 2
numerical_accuracy <- 0.00000000001

simulated_preference_pdf <- 100

# minimum error
preference_error_threshold <- 1

while (simulated_preference_pdf > numerical_accuracy) {
  
  simulated_preference_pdf <- preference_error_threshold * exp(-preference_strength * preference_error_threshold)

  print(preference_error_threshold)
  print(simulated_preference_pdf)

  preference_error_threshold <- preference_error_threshold + 1;
}

#preference_strength * exp(-preference_strength * preference_error_sum)
#dexp(preference_error_sum, preference_strength)


# Sample maximum error values, growing exponentially
max_err <- c(738, 6123, 863860)
case <- 3

#preference_strength <- -log(1 - 0.8) / max_err;

# Function to calculate preference strength based on max error
calc_preference_strength <- function(max_err) {
  # Parameters to scale and adjust the transformation, as needed
  base_strength <- 5  # Base or maximum possible strength
  scaling_factor <- 100000  # Adjusts the sensitivity to max_err changes
  
  # Calculate preference strength inversely related to log of max_err
  strength <- base_strength - log(max_err + 1) / log(scaling_factor + 1) * (base_strength - 1)
  
  # Ensure strength is within the range [1, base_strength]
  return(max(1, min(base_strength, strength)))
}

# Example of calculating and plotting for the first max_err value
preference_strength <- calc_preference_strength(max_err[case])

error_values <- seq(0, max_err[case], by = 1)
pdf_values <- dexp(error_values, rate = preference_strength)

ggplot(data = data.frame(error_values, pdf_values), aes(x = error_values, y = pdf_values)) +
  geom_line(color = "blue") +
  ggtitle(paste("Exponential Distribution with Preference Strength =", preference_strength))
...
Q_effects <- colMeans(result$Q_effects[,3,])
...
for (i in 1:100) {
  print((1 - result$Q_effects)[1,] %*% result$Q_diffs[i,])
}
...
# fsins odotusarvo.. rakenna tämä 

rowSums(result$Q_effects[,3,])
mean(result$Y_mu[,3])

...
sysdimet_lp_plots <- function(person_vector, plot_path) {
  ...
  for (i in person_vector) {
  
      accepted_intake.df <- readRDS(paste0("patient_summary/sysdimet/gamma_qr_mv/simulationX50_B50_A50_I50/details/accepted_intake_",i,".rds"))
  
      # Get personal normal ranges 
      subject_code <- kontrolli[i]
      personal_info <- head(sysdimet[sysdimet$SUBJECT_ID == subject_code,],1)
      recommeded_concentrations <- mebn.get_personal_sysdimet_guidelines(personal_info)
      ...
      curr_fshdl <- concinfo.df[concinfo.df$person_id == i,]$current_fshdl
      curr_fsldl <- concinfo.df[concinfo.df$person_id == i,]$current_fsldl
      curr_fsins <- concinfo.df[concinfo.df$person_id == i,]$current_fsins
      curr_fskol <- concinfo.df[concinfo.df$person_id == i,]$current_fskol
      curr_fpgluk <- concinfo.df[concinfo.df$person_id == i,]$current_fpgluk
  
      pred_fshdl <- concinfo.df[concinfo.df$person_id == i,]$fshdl
      pred_fsldl <- concinfo.df[concinfo.df$person_id == i,]$fsldl
      pred_fsins <- concinfo.df[concinfo.df$person_id == i,]$fsins
      pred_fskol <- concinfo.df[concinfo.df$person_id == i,]$fskol
      pred_fpgluk <- concinfo.df[concinfo.df$person_id == i,]$fpgluk
      
      bound_limit <- 0.5
      
      fshdl_accepted <- concinfo.df[concinfo.df$person_id == i,]$fshdl_lowerbound > bound_limit & concinfo.df[concinfo.df$person_id == i,]$fshdl_upperbound > bound_limit
      fsldl_accepted <- concinfo.df[concinfo.df$person_id == i,]$fsldl_lowerbound > bound_limit & concinfo.df[concinfo.df$person_id == i,]$fsldl_upperbound > bound_limit
      fsins_accepted <- concinfo.df[concinfo.df$person_id == i,]$fsins_lowerbound > bound_limit & concinfo.df[concinfo.df$person_id == i,]$fsins_upperbound > bound_limit
      fskol_accepted <- concinfo.df[concinfo.df$person_id == i,]$fskol_lowerbound > bound_limit & concinfo.df[concinfo.df$person_id == i,]$fskol_upperbound > bound_limit
      fpgluk_accepted <- concinfo.df[concinfo.df$person_id == i,]$fpgluk_lowerbound > bound_limit & concinfo.df[concinfo.df$person_id == i,]$fpgluk_upperbound > bound_limit
  ...
  accepted_intake.df <- readRDS(paste0("patient_summary/sysdimet/gamma_qr_mv/simulation",simulation,"/details/accepted_intake_",i,".rds"))
...
persons <- 10
person_vector <- 5:(10-1)

sysdimet_lp_plots(person_vector, "figures/sysdimet_concentrations_exmple.pdf")

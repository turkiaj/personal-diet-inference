...
max(exp(result$in_concentration_range_lpdf))
max(exp(result$preference_lpdf))

# Tämä plotti paljastaa ongelman: 
# - suurin massa sampleista on pienen preferenssin kohdalla
# - preferenssin huippu suosii pieniä dvit-arvoja, mutta sellaisia saanteja on määrällisesti vähemmän eli ne ovat vähemmän todennäköisiä

ggplot(as.data.frame(recommendation_intake.df)) +
  geom_point(aes(x = dvit, y = (1-in_range)*exp(in_concentration_range) + in_range * exp(preference)), color="red") +
  geom_point(aes(x = dvit, y = (1-in_range)*exp(in_concentration_range)), color="blue") 

# P1 current kuitu 26.4
# P1 current dvit 6.1375

recommendation_intake.df$linoliepros
recommendation_intake.df$linoleeniepros

plot(recommendation_intake.df$linoliepros, recommendation_intake.df$preference)
plot(recommendation_intake.df$kuitu, recommendation_intake.df$preference)

result$dvit <- intake_proposals.df$dvit 
result$kuitu <- intake_proposals.df$kuitu
result$preference_lpdf <- intake_proposals.df$preference

# Tämä on nyt tärkein plotti. Jakauma pitäisi saada niin, että suurempiin virheisiin mentäessä jakauma jyrkkenee ja loivenee pienemmillä jolloin sielä on liikkumavaraa
#ggplot(as.data.frame(result)) + geom_point(aes(x = preference_error_sum, y = preference_lpdf, color=1-dvit))

# Näillä arvoilla tulee nyt hyvä kasa
ggplot(as.data.frame(result)) +
  geom_point(aes(x = dvit, y = preference_lpdf))

ggplot(as.data.frame(result)) +
  geom_point(aes(x = in_concentration_range_lpdf, y = preference_lpdf,  color = 1-dvit))
...
intake.df <- readRDS("patient_summary/sysdimet/gamma_qr_mv/simulationX50_B50_A50_I50_flatconc/recommendation_summary_with_concpreds_and_current.rds")
...
#personal_graph1 <- read.graph("graphs/sysdimet/gamma_qr_mv/1/personal_graph.graphml", "graphml")
#personal_graph3 <- read.graph("graphs/sysdimet/gamma_qr_mv/3/personal_graph.graphml", "graphml")
personal_graph6 <- read.graph("graphs/sysdimet/gamma_qr_mv/6/personal_graph.graphml", "graphml")

nutrients <- V(personal_graph6)[V(personal_graph6)$type == 100 | V(personal_graph6)$type == 200]

name_lookup <- data.frame(Name = nutrients$name) %>% inner_join(sysdimet_datadesc, by = join_by(Name))

layout <- mebn.plot_personal_effects(personal_graph6, 60, name_lookup$ShortDescription, graph_layout = NULL)

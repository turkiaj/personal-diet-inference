...
# - data parameters
graph_dir <- "graphs/sysdimet/mv3_cross_two_levels/"
...
for (person_id in 1:37) {
  ...
  subject_code <- levels(sysdimet$potilas)[person_id]
  treatment <- unique(sysdimet[sysdimet$potilas == subject_code,]$hoitomuoto)
  
  # Normal ranges of concentration are personalized
  personal_info <- head(sysdimet[sysdimet$potilas == subject_code,],1)
  recommeded_concentrations <- mebn.get_personal_target_guidelines(personal_info, patient_in_sysdimet = TRUE)
  ...
  {
    saveRDS(accepted_intake.df %>% as.data.frame(), paste0("patient_summary/sysdimet/mv3_cross_two_levels/simulation",simulation,"/details/accepted_intake_",person_id,".rds"))
    ...
    saveRDS(recommendation_intake.df %>% as.data.frame(), paste0("patient_summary/sysdimet/mv3_cross_two_levels/simulation",simulation,"/details/recommendation_intake_",person_id,".rds"))
  ...
  if (FALSE) {
    ...
    ggsave(paste0("figures/sysdimet/personal_recommendations/log_probability_",person_id,".pdf"), plot = lp_plot)
...
saveRDS(patient_summary, paste0("patient_summary/sysdimet/mv3_cross_two_levels/simulation",simulation,"/recommendation_summary.rds"))
...
```{r concentration_predictions}
  ...
  patient_summary <- readRDS("patient_summary/sysdimet/mv3_cross_two_levels/simulationX50_B50_A50_I50/recommendation_summary.rds")

  for (subject_id in 1:37) {
    
    print(subject_id)

    graph_dir <- paste0("graphs/sysdimet/mv3_cross_two_levels/",subject_id)
  ...
  saveRDS(patient_summary, paste0("patient_summary/sysdimet/mv3_cross_two_levels/simulationX50_B50_A50_I50/recommendation_summary_with_concpreds.rds"))
...
for (person_id in 1:37) {
  ...
  graph_dir <- paste0("graphs/sysdimet/mv3_cross_two_levels/",person_id)
...
recommendation_with_concpreds.df <- readRDS("patient_summary/sysdimet/mv3_cross_two_levels/simulationX50_B50_A50_I50/recommendation_summary_with_concpreds.rds")
...
saveRDS(recommendation_with_concpreds_and_current.df, paste0("patient_summary/sysdimet/mv3_cross_two_levels/simulationX50_B50_A50_I50/recommendation_summary_with_concpreds_and_current.rds"))
...
concinfo.df <- readRDS("patient_summary/sysdimet/mv3_cross_two_levels/simulationX50_B50_A50_I50/recommendation_summary_with_concpreds_and_current.rds")

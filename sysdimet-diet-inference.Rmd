...
fshdl_accepted <- FALSE

#                                   colorspace::diverge_hcl(7, c = 240, l = c(90, 50), power = 1),

lp_mid <- mean(recommendation_intake.df$lp) * 0.40
...
sysdimet_lp_plots <- function(subject_code_vector, plot_path) {
  ...
  for (subject_code in subject_code_vector[1:(persons-1)]) {
      ...
      conc_plot <- grid.arrange(conc_plot1, conc_plot2, conc_plot3, conc_plot4, conc_plot5, ncol=5, padding=0)
  ...
  conc_plot1 <- ggplot(accepted_intake.df) +
    ...
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
    theme(axis.text.y = element_blank(), axis.ticks.y = element_blank()) +
    labs(x = "fS-HDL (mmol/l)", y = subject_code)
...
sysdimet_lp_plots(subjects_with_failed_concs$subject_code, "figures/sysdimet_fixed_concentrations.pdf")
  ...
  #person_id <- concinfo.df[concinfo.df$subject_code == 44,]$person_id
  #print(paste0("2. person_id: ", person_id))
...
concinfo.df[concinfo.df$subject_code %in% subjects_with_failed_concs$subject_code,]$fsldl_upperbound
concinfo.df[concinfo.df$subject_code %in% subjects_with_failed_concs$subject_code,]$fsins_upperbound
concinfo.df[concinfo.df$subject_code %in% subjects_with_failed_concs$subject_code,]$fpgluk_upperbound

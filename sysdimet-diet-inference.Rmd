...
colnames(patient_summary) <- c("person_id", "subject_code", "accepted", paste0(queried_nodes), paste0(queried_nodes, "_lCI"), paste0(queried_nodes, "_uCI"), assumedtargets$Name)
...
```{r sysdimet_lp_plots, cache=FALSE, eval=TRUE, echo=FALSE,message=FALSE, warning=FALSE, fig.width=8, fig.height=20, fig.keep='last'}
library(ggplot2)
library(gridExtra)
...
fsldl_scale_min <- min(2.0, concinfo.df$current_fsldl[1:persons])-0.1
...
fskol_scale_min <- min(3.9, concinfo.df$current_fskol[1:persons])-0.1
...
for (i in 1:(persons-1)) {
    ...
    accepted <- concinfo.df[concinfo.df$person_id == i,]$accepted
    if (accepted) conc_palette <- c("#00337C", "#557C8C", "#99BB99", "#03C988") else conc_palette <- c("#13005A", "#9B0000", "#FFB5B5", "#FF6961")
    ...
    conc_plot1 <- ggplot(accepted_intake.df) +
      ...
      scale_color_gradientn(colours = conc_palette) +
      theme_light() +

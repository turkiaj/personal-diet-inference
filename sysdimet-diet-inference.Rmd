...
bound.df <- data.frame(proposal=1:dim(result$Y_mu)[1])
bound.df$bound <- result$Y_bound[,10]
bound.df$lp <- result$lp__

ggplot(bound.df) +
  geom_point(aes(y=bound, x=proposal), show.legend=FALSE)     

colMeans(result$Y_bound)
...
sysdimet_lp_plots <- function(subject_code_vector, plot_path) {
  ...
  fshdl_scale_max <- max(2.1, concinfo.df$current_fshdl[1:persons])+0.1
  ...
  for (subject_code in subject_code_vector[1:(persons-1)]) {
      ...
      conc_plot1 <- ggplot(accepted_intake.df) +
        ...
        scale_x_continuous(limits=c(fshdl_scale_min, fshdl_scale_max), breaks=c(0.8,1.0,2.1,2.8)) +
  ...
  full_conc_plot <- grid.arrange(grobs = conc_plots, ncol=1, padding=0, heights=c(rep(1, plot_index),1.2))
...
person_vector <- 1:14
sysdimet_lp_plots(person_vector, "figures/sysdimet_concentrations_all1.pdf")

person_vector <- 16:34
sysdimet_lp_plots(person_vector, "figures/sysdimet_concentrations_all2.pdf")

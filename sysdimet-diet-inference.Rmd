...
# clustering
feature_index <- c(1:nrow(sysdimet_predictors))
personal_effects <- matrix(NA, ncol=length(feature_index)*nrow(sysdimet_targets), nrow=length(levels(sysdimet$SUBJECT_ID)))
...
for (subject_code in recommendation_with_concpreds.df$subject_code) {
  ...
  for (i in 1:length(queried_nodes))
  {
    ...
    # collect personal effects for clustering
    for (t in sysdimet_targets$Name)
    {
      eff_name <- paste0("personal_",queried_nodes[i],"_",t)
      b <- V(personal_graph)[eff_name]$value
      
      effects <- c(effects, b)
      effect_names <- c(effect_names, eff_name)
    }
  }
...
colnames(personal_effects) <- effect_names
...
# ! TODO: Make this mean over all concentrations
...
p <- ggplot(cumulative_concs) +
   geom_point(aes(x = predicted_concentration, y = nutrient, color = subject_id), size = 0.7, show.legend = TRUE) +
   geom_path(aes(x = predicted_concentration, y = nutrient, group = subject_id, color = subject_id), linewidth=0.3, show.legend = FALSE) +
   geom_vline(data = limit_data, aes(xintercept = upper_limits), linetype = "dashed", color = "#CCCCCC") +
   geom_vline(data = limit_data, aes(xintercept = lower_limits), linetype = "dashed", color = "#CCCCCC") +
...
# highlight <- c(15, 17)
# 
# colors <- setNames(rep("lightgray", length(unique(cumulative_concs$person_id))), unique(cumulative_concs$person_id))
# colors[as.character(highlight)] <- "red"
# 
# p <- ggplot(cumulative_concs) +
#   geom_point(aes(x = predicted_concentration, y = nutrient, color = factor(person_id)), size = 1.0, show.legend = TRUE) +
#   geom_path(aes(x = predicted_concentration, y = nutrient, group = factor(person_id), color = factor(person_id)), linewidth=0.3, show.legend = FALSE) +
#   geom_vline(data = limit_data, aes(xintercept = upper_limits), linetype = "dashed", color = "black") +
#   geom_vline(data = limit_data2, aes(xintercept = upper_limits), linetype = "dashed", color = "black") +
#   geom_vline(data = limit_data, aes(xintercept = lower_limits), linetype = "dashed", color = "black") +
#   facet_wrap(~concentration, scales = "free_x", labeller = labeller(concentration = conc_labels)) +
#   scale_y_discrete(labels = nutrient_labels) +
#   theme_bw() +
#   labs(x = element_blank(), y = element_blank()) +
#   scale_color_manual(values = colors) # Apply the manual color scale
# p  

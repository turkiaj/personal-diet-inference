...
```{r build_effect_summary, echo=FALSE, eval=FALSE}
...
patients <- levels(kontrolli$SUBJECT_ID)
...
for (person_id in 1:length(patients)) {
  ...
  for (c in 1:dim(sysdimet_targets)[1])
  {
    target_column <- sysdimet_targets[c,]
    target_name <- as.vector(target_column$Name)
    
    predictor_names <- as.vector(sysdimet_predictors$Name)
    
    for (p in 1:dim(sysdimet_predictors)[1])
    {

...
for (person_id in 1:1) {
  ...
  colnames(intake_proposals.df) <- c("proposal", "lp", "preference", "in_concentration_range", "boundsum", assumedtargets$Name, queried_nodes)
...
ggplot(as.data.frame(result)) +
  geom_point(aes(y=in_range*exp(preference_lpdf)*200, x=exp(in_concentration_range_lpdf), color=dvit), show.legend=FALSE) +
  scale_color_gradientn(colours = colorspace::diverge_hcl(7))

mean(intake_proposals.df$dvit)  

plot(result$softbound_sum, result$in_range)
plot(result$softbound_sum, exp(result$in_concentration_range_lpdf))
plot(result$preference_error_sum, exp(result$preference_lpdf))

max(exp(result$in_concentration_range_lpdf))
max(exp(result$preference_lpdf))

ggplot(as.data.frame(result)) +
  geom_point(aes(x = preference_error_sum, y = result$in_range * (exp(result$preference_lpdf) * 200), color=result$in_range))

ggplot(as.data.frame(result)) +
  geom_point(aes(x = softbound_sum, y = exp(result$in_concentration_range_lpdf), color=result$softbound_sum))

ggplot(as.data.frame(result), aes(x=exp(result$in_concentration_range_lpdf))) +
  geom_density(color="darkblue", fill="lightblue")

ggplot(as.data.frame(result), aes(x=exp(result$preference_lpdf))) +
  geom_density(color="darkblue", fill="lightblue")

ggplot(as.data.frame(result), aes(x=result$lp)) +
  geom_density(color="darkblue", fill="lightblue")
...
ggplot(as.data.frame(result)) +
  geom_point(aes(x = dvit, y = result$in_range * (exp(result$preference_lpdf) * 200), color=1-result$preference_error_sum))
...
ggplot(recommendation_intake.df) +
  geom_point(aes(y=fshdl, x=dvit, color=boundsum), show.legend=FALSE) +
  geom_vline(xintercept = fpgluk_recommendations, linetype="solid", color = "black", linewidth=0.4) +
  scale_color_gradientn(colours = colorspace::diverge_hcl(7))
...
stan_model <- stan(file="mebn/joint_density_demo.stan",model_name="demo", warmup = 500, iter=1000, chains=4, data=data_list, seed=483892929)

result <- rstan::extract(stan_model)
df <- as.data.frame(result)

ggplot(df, aes(x=lp__)) +
  geom_density(color="darkblue", fill="lightblue")
...
nutrients <- V(personal_graph1)[V(personal_graph1)$type == 100 | V(personal_graph1)$type == 200]

name_lookup <- data.frame(Name = nutrients$name) %>% inner_join(sysdimet_datadesc, by = join_by(Name))

layout <- mebn.plot_personal_effects(personal_graph1, 60, name_lookup$ShortDescription, graph_layout = NULL)

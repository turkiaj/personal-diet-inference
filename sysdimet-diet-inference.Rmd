...
# linoleeniepros	linoleeni	Alpha-linolenic acid
# beta = -7.94

org_mean <- mean(sysdimet$linoleeniepros)
org_sd <- sd(sysdimet$linoleeniepros)

ggplot(sysdimet, aes((linoleeniepros-org_mean)/org_sd, fsins, group=SUBJECT_ID, color=SUBJECT_ID)) +
...
# linoliepros	linoliepros	Linoleic acid
# beta = 7.52

org_mean <- mean(sysdimet$linoliepros)
org_sd <- sd(sysdimet$linoliepros)

ggplot(sysdimet, aes((linoliepros-org_mean)/org_sd, fsins, group=SUBJECT_ID, color=SUBJECT_ID)) +
  geom_point(show.legend = FALSE) +
  geom_smooth(method = lm, se = FALSE, linewidth=0.5, show.legend = FALSE)
...
pe <- rstan::summary(target_blmm, pars="personal_effect")
pe

beta <- rstan::summary(target_blmm, pars="beta")
...
target_blmm <- mebn.get_localfit("fshdl_fsldl_fsins_fskol_fpgluk", "models/sysdimet/BLMM_gamma_qr_mv/")
...
for (i in 1:obs)
{
  ...
  for (target in 1:nrow(sysdimet_targets))
  {
    rep_response <- params$Y_rep[,target,i]
    
    target_name <- sysdimet_targets[target,]$Name

    # true value for observation
    true_response <- sysdimet[i,][target_name]
...
colnames(NRMSE) <- sysdimet_targets$Name
...
for (person_id in 1:length(kontrolli)) {
  ...
  if (check_failed_concs(graph_person_id, personal_info, recommeded_concentrations) == TRUE) { 
      ...
      # This does not work with any hyperparameters
      # [1] "Simulating person_id 10"
      # [1] "Simulating graph_person_id 28"
      # [1] "Simulating subject_code S121"
      ...
      intake_model <- mebn.Query(reaction_graph = personal_graph,
                               ...
                               repeat_only = 0,
                               verbose = 1,
                               l1 = 8,      # limit_steepness: Steepness parameter for individual soft bounds
                               l2 = 15,     # limit_requirement: Requirement for meeting all bounds
      ...
      intake_proposals.df$within_limits_lpdf <- result$within_limits_lpdf
      intake_proposals.df$limitsum <- result$softlimit_sum
      intake_proposals.df$in_range <- result$in_range
      
      # Data frame to store contributions
      ...
      for (node_index in 1:nrow(assumedtargets)) {
        ...
        temp.df$concentration <- node_name
	      ...
	      # We only show the expected values of contributions but whole distribution is available
        temp.df$contributions <- colMeans(result$Q_contributions[,node_index,])

	      # Baseline of fixed effects is included so that accumulated contributions result the final concentration 
      ...
      colnames(intake_proposals.df) <- c("proposal", "lp", "preference", "within_limits_lpdf", "limitsum", "in_range", assumedtargets$Name, queried_nodes)
      ...
      if (nrow(recommendation_intake.df) > 0)
      {
          ...
          personal_intake <- c(graph_person_id,
                               ...
                               apply(result$Y_limit[,seq(1, nrow(assumedtargets))], MARGIN=c(2), max),
                               apply(result$Y_limit[,2*seq(1, nrow(assumedtargets))], MARGIN=c(2), max)
                               )
      } 
      else
      {
...
effect_summary <- personal_effects %>% 
  ...
  arrange(desc(total_personal)) %>% top_n(100)
...
kable(effect_summary_table.df[c(6,2:5)], "latex", escape = F, booktabs = T, row.names = FALSE, digits = 2,align="lccccccccccc",
  ...
  row_spec(0,bold=TRUE) %>% save_kable(file = "tables/sysdimet_effects.pdf", keep_tex = TRUE)

#effect_summary_table.df[effect_summary_table.df$nutrient == "Sucrose",]

effect_summary_table.df[effect_summary_table.df$nutrient == "EPS-fatty acid",][c("effect_description", "min_personal", "max_personal")]

```

```{r}
# Function to remove nearly identical points
library(dplyr)

# Function to remove overlapping points with proportional tolerance
remove_overlapping_points <- function(data, x_col, y_col, tolerance_ratio = 0.01) {
  # Ensure x_col and y_col are interpreted as column names
  x_col <- rlang::ensym(x_col)
  y_col <- rlang::ensym(y_col)
  
  # Calculate tolerances based on the range of x and y
  x_range <- diff(range(data[[rlang::as_string(x_col)]]))
  y_range <- diff(range(data[[rlang::as_string(y_col)]]))
  
  tolerance_x <- x_range * tolerance_ratio
  tolerance_y <- y_range * tolerance_ratio
  
  # Perform the deduplication
  data %>%
    mutate(
      x_rounded = round(!!x_col / tolerance_x),
      y_rounded = round(!!y_col / tolerance_y)
    ) %>%
    group_by(x_rounded, y_rounded) %>%
    slice(1) %>%  # Keep the first point in each group
    ungroup() %>%
    select(-x_rounded, -y_rounded)  # Remove helper columns
}
...
sysdimet_lp_plots <- function(subject_code_vector, plot_path) {
  ...
  for (subject_code in subject_code_vector[1:(persons-1)]) {
      ...
      if (file.exists(rec_file)) {
        ...
        subsampled_intake.df <- remove_overlapping_points(accepted_intake.df, fshdl, lp, subsampling_tolerance)
        
        conc_plot1 <- ggplot(subsampled_intake.df) +
        ...
        subsampled_intake.df <- remove_overlapping_points(accepted_intake.df, fsldl, lp, subsampling_tolerance)
  
        conc_plot2 <- ggplot(subsampled_intake.df) +
        ...
        subsampled_intake.df <- remove_overlapping_points(accepted_intake.df, fsins, lp, subsampling_tolerance)
    
        conc_plot3 <- ggplot(subsampled_intake.df) +
        ...
        subsampled_intake.df <- remove_overlapping_points(accepted_intake.df, fskol, lp, subsampling_tolerance)
    
        conc_plot4 <- ggplot(subsampled_intake.df) +
        ...
        subsampled_intake.df <- remove_overlapping_points(accepted_intake.df, fpgluk, lp, subsampling_tolerance)
    
        conc_plot5 <- ggplot(subsampled_intake.df) +
...
sysdimet_lp_plots(subjects_with_failed_concs[1:5,]$subject_code, "figures/sysdimet_fixed_concentrations.pdf")

```


```{r sysdimet_lp_article_plot, cache=FALSE, eval=TRUE, echo=FALSE,message=FALSE, warning=FALSE, fig.width=7, fig.height=10, fig.keep='last'}
# Only patients with non-optimal concentrations are included in the recommendation

subjects_with_failed_concs <- readRDS(paste0("patient_summary/sysdimet/gamma_qr_mv/simulation",simulation,"/recommendation_summary_with_concpreds_and_current.rds"))

#sysdimet_lp_plots(subjects_with_failed_concs[1:5,]$subject_code, "figures/sysdimet_fixed_concentrations.pdf")
sysdimet_lp_plots(subjects_with_failed_concs[6:16,]$subject_code, "figures/sysdimet_fixed_concentrations_rest.pdf")
```
...
cumulative_concs$is104 <- cumulative_concs$subject_id %in% c(104) 

cumulative_concs$highlight_alpha <- ifelse(cumulative_concs$example, 0.9, 0.8)
cumulative_concs$highlight_width <- ifelse(cumulative_concs$example, 0.7, 0.65)
cumulative_concs$highlight_type <- ifelse(cumulative_concs$is104, 5, 1)
...
p <- ggplot(cumulative_concs) +
   ...
   geom_path(aes(x = predicted_concentration, y = nutrient, group = subject_id, color = subject_id, alpha = highlight_alpha, linewidth=highlight_width, linetype = highlight_type), show.legend = FALSE) +
...
ggplot(sysdimet, aes(y=epa, x=fshdl), group = SUBJECT_ID) +

...
scode <- "parameters { real y;} model { y ~ uniform(-100,100); target += y; print(target()); }"

fit1 <- stan(model_code = scode, iter = 500, verbose = FALSE)
...
#for (person_id in 1:37) {
  ...
  colnames(intake_proposals.df) <- c("proposal", queried_nodes)
...
exceeding <- seq(6,0,by=-0.1)
residuals <- seq(7,0.1,by=-0.1)

lp <- c(100-exceeding, 100+1/residuals)

proposal <- 1:length(lp)
df <- data.frame(proposal=proposal, lp=lp)
...
```{r party, cache=FALSE, eval=TRUE, echo=FALSE,message=FALSE, warning=FALSE, fig.height=8, fig.cap="The figure is plotted with ggplot2 package for R language (v 3.3.5, https://ggplot2.tidyverse.org)."}
library(ggplot2)
library(gridExtra)
library(ggrepel)
library(dplyr)
...
for (person_id in 36:36) {
  ...
  if (file.exists(accepted_intake)) {
    ...
    # Plotting
    cond_intake_names <- colnames(accepted_intake.df)[7:ncol(accepted_intake.df)]
    ...
    for (i in 1:number_of_cond_intake)
    {
      ...
      RI <- as.numeric(variable_desc$Recommendation)
      
      intake_plot <- ggplot(subset(accepted_intake.df, lowest_conc_prob >= 0.50)) + 
        geom_density(aes(x = !!intake_symbol), fill = "#0000FF", colour = "#0000FF", alpha = 0.2, show.legend = FALSE) +
        labs(x = paste0(variable_desc$Description), y = "") +
        geom_vline(xintercept = RI, linetype="dashed", color = "black", size=0.5) +
    ...
    #     theme(axis.text.y = element_blank())
    # 
    # prot_plot <- ggplot(subset(accepted_intake.df, lowest_conc_prob >= 0.50)) + 
    #     geom_density(aes(x = prot), fill = "#0000FF", colour = "#0000FF", alpha = 0.2, show.legend = TRUE) +
    #     labs(x = "Protein, E%", y = "") +
    #     scale_x_continuous(limits=c(0,22.9), breaks=c(0,10,20,25)) +  
    #     theme_bw() +
    #     theme(axis.text.y = element_blank())
...
#                    c(2,5))
    ...
    #reasoning_plot <- grid.arrange(intake_plots[[1]], intake_plots[[2]], layout_matrix = lay, padding=0)
    reasoning_plot <- grid.arrange(intake_plots[[1]], intake_plots[[2]], intake_plots[[3]], intake_plots[[4]], intake_plots[[5]], layout_matrix = lay, padding=0)
    #reasoning_plot <- grid.arrange(intake_plots[[1]], intake_plots[[2]], intake_plots[[3]], pk_plot, fppi_plot, palb_plot, layout_matrix = lay, padding=0)
    #reasoning_plot <- grid.arrange(intake_plots[[1]], intake_plots[[2]], pk_plot, fppi_plot, palb_plot, layout_matrix = lay, padding=0)

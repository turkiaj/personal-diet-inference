...
```{r}
dialysis_conc_predictors.df <- dialysis_predictors %>% 
...
kable(dialysis_conc_predictors.df, format = "html", escape = F, booktabs = T, row.names = FALSE, digits = 2,
...
for (person_id in 1:37) {
  ...
  intake_model <- mebn.Query(reaction_graph = personal_graph,
                           ...
                           general_RI = RI,
                           stan_model_file = "diet/recommendation_with_utility_invlogit.stan",
...
# TODO: Get amount of digits from 
# cond_nutrients$Decimals

# TODO: Add bolding for over 30% increase or decrease

# TODO: Add CI columns with this
...
recommendation_table.df <- intake.df %>%  
                              mutate(across(.cols = recommendation,
                                            ...
                                            .names = "{.col}_color")) %>%
                              select(c("person_id", recommendation, paste0(recommendation,"_color")))
    
# recommendation_table.df <- intake.df[1,] %>%
#   mutate(across(.cols = recommendation,
#                 .fns = ~case_when(
#                   get(glue::glue("current_{cur_column()}")) < .x ~ "up",
#                   get(glue::glue("current_{cur_column()}"))  > .x ~ "down",
#                   get(glue::glue("current_{cur_column()}"))  == .x ~ paste0("-"),
#                 ),
#                 .names = "{.col}_icon")) %>%
#   select(c("person_id", recommendation))

# add empty colnames for icons
#table_colnames <- c(rbind(table_colnames, " "))

# remove last empty
#table_colnames <- table_colnames[1:length(table_colnames)-1]

kable(recommendation_table.df[1:14], format = "html", escape = F, booktabs = T, row.names = FALSE, digits = 2, col.names = table_colnames,
   caption = "Personally recommended intake (RI) for renal patients with lower and upper limits of recommendations.") %>%
    kable_styling(latex_options="scale_down") %>%  
    kable_styling(latex_options = c("basic", "condensed"), full_width = TRUE) %>%
   row_spec(0,bold=TRUE) %>%
   column_spec(2, color = recommendation_table.df[,15]) %>%
   column_spec(3, color = recommendation_table.df[,16]) %>%
   column_spec(4, color = recommendation_table.df[,17]) %>%
   column_spec(5, color = recommendation_table.df[,18]) %>%
   column_spec(6, color = recommendation_table.df[,19]) %>%
   column_spec(7, color = recommendation_table.df[,20]) %>%
   column_spec(8, color = recommendation_table.df[,21]) %>%
   column_spec(9, color = recommendation_table.df[,22]) %>%
   column_spec(10, color = recommendation_table.df[,23]) %>%
   column_spec(11, color = recommendation_table.df[,24]) %>%
   column_spec(12, color = recommendation_table.df[,25]) %>%
   column_spec(13, color = recommendation_table.df[,26]) %>%
   column_spec(14, color = recommendation_table.df[,27]) 

# kable(recommendation_table.df, format = "latex", escape = F, booktabs = T, row.names = FALSE, digits = 2, col.names = table_colnames,
#    caption = "Personally recommended intake (RI) for renal patients with lower and upper limits of recommendations.") %>%
#    kable_styling(latex_options="scale_down") %>%  
#    kable_styling(latex_options = c("basic", "condensed"), full_width = FALSE) %>%
#    row_spec(0,bold=TRUE) %>%
...
kable(recommendation_table.df[1:14], format = "latex", escape = F, booktabs = T, row.names = FALSE, digits = 2, col.names = table_colnames,
   caption = "Personally recommended intake (RI) for renal patients with lower and upper limits of recommendations.") %>%
    kable_styling(latex_options="scale_down") %>%  
    kable_styling(latex_options = c("basic", "condensed"), full_width = TRUE) %>%
   row_spec(0,bold=TRUE) %>%

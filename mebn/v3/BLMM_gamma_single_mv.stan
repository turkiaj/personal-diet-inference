...
parameters { 
  ...
  // Subject level random effects have their of Choleskies with same size
    ...
    for (m in 1:v)
    {
      for (n in 1:N-NH)
      {
        // - row n is picked from ZM_t and multiplied with column of coefs from a patient
         mu[vn] = mu[vn] + ZM_t[vn] * b_stack[subject[n]];
         vn += 1;
      }
    }
...
generated quantities {
  ...
  // - left out for getting smaller model
  ...
  // Unstack Y_rep to separate columns
  {
    ...
    for (m in 1:v)
    {
      ...
      {
        // - row n is picked from ZM_t and multiplied with column of coefs from a patient
         mu_hat[vn] = mu_hat[vn] + ZM_t[vn] * b_stack[subject[n]];
         vn += 1;
      }
    }

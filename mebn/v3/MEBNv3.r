...
mebn.fully_connected_bipartite_graph <- function(datadesc)
{
  ...
  reaction_graph <- reaction_graph + vertices(as.vector(assumed_targets$Name), 
                                              label=as.vector(assumed_targets$Name), 
                                              description=as.vector(assumed_targets$ShortDescription),
                                              type=200,
                                              color = "#74aaf2", 
                                              size = 1,
                                              shape = "circle")
  ...
  reaction_graph <- reaction_graph + vertices(as.vector(predictor_columns$Name), 
                                              label=as.vector(predictor_columns$Name), 
                                              description=as.vector(predictor_columns$ShortDescription),
                                              type=100,
                                              color = "#3cd164", 
                                              size = 1, 
                                              shape = "circle")
  ...
  params <- within(list(),
                   {
                     ...
                     Y <- Y
                     Z <- X     
                     n_s <- length(levels(inputdata[[group_column]]))
                     subject <- as.integer(inputdata[[group_column]])
                     holdout <- targetdata
                     offset <- 100
                   })
...
mebn.Query <- function(reaction_graph, graph_dir, query, queried_nodes, proposal_lowerlimits, proposal_upperlimits, general_RI, conc_lower_limits, conc_upper_limits, stan_model_file, X_point_est = "mean", beta_point_est = "mean", param_point_est = "mean", posterior_samples = 100, repeat_only = 0)
{
  library(rstan)
  library(igraph)
  library(stringr)
  ...
  params <- within(list(),
                   {
                     ...
                     proposal_lowerlimits <- proposal_lowerlimits

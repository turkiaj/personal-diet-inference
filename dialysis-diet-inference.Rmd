...
#    select(Predictor, Range, RI, Sysdimet, Dialysis)
    select(Predictor, Range, Sysdimet, Dialysis)
...
kable(example_predictors.df, format = "latex", escape = F, booktabs = T, row.names = FALSE, 
   ...
   col.names = c("Nutrient", "General recommendation", "Sysdimet", "Dialysis")) %>%
...
for (person_id in 1:37) {
  ...
  saveRDS(intake_proposals.df %>% as.data.frame(), paste0("patient_summary/dialysis/mv3_cross_two_levels/simulation",simulation,"/details/accepted_intake_",person_id,".rds"))
  ...
  if (nrow(recommendation_intake.df) > 0)
  {
      ...
      personal_intake <- c(person_id,
                           ...
                           colMeans(result$Y_bound[,seq(1, nrow(assumedtargets))]),
                           colMeans(result$Y_bound[,2*seq(1, nrow(assumedtargets))])
                           )
  } 
  else
  {
...
conc_plot2 <- ggplot(recommendation_intake.df) +
  geom_point(aes(y=lp, x=fppi, color=preference), show.legend=FALSE) +
...
conc_plot3 <- ggplot(recommendation_intake.df) +
  geom_point(aes(y=lp, x=palb, color=preference), show.legend=FALSE) +
...
dialysis_lp_plots <- function(subject_code_vector, plot_path) {
  ...
  for (subject_code in subject_code_vector[1:(persons-1)]) {
      ...
      accepted_intake.df <- readRDS(paste0("patient_summary/dialysis/mv3_cross_two_levels/simulationX50_B50_A50_I50/details/accepted_intake_",person_id,".rds"))
  ...
  accepted_intake.df <- readRDS(paste0("patient_summary/dialysis/mv3_cross_two_levels/simulation",simulation,"/details/accepted_intake_",person_id,".rds"))
...
```{r dialysis_lp_article_plot, cache=FALSE, eval=TRUE, echo=FALSE,message=FALSE, warning=FALSE, fig.width=8, fig.height=9, fig.keep='last'}
source("mebn/v3/MEBNv3.r")

process_limits <- function(personal_info) {
  
  subject_code <- as.numeric(as.character(personal_info[["potilas"]]))
  ...
  as.numeric(c(subject_code, pk_min, fppi_min, palb_min, pk_max, fppi_max, palb_max))
...
colnames(conc_limits) <- c("subject_code", "pk_min", "fppi_min", "palb_min", "pk_max", "fppi_max", "palb_max")

concinfo.df <- readRDS("patient_summary/dialysis/mv3_cross_two_levels/simulationX50_B50_A50_I50/recommendation_summary_with_concpreds_and_current.rds")

dailysis_with_failed_concs <- concinfo.df %>%
  inner_join(conc_limits, by = "subject_code") %>%
  filter(current_pk < pk_min | current_pk > pk_max |
         current_fppi < fppi_min | current_fppi > fppi_max |
         current_palb < palb_min | current_palb > palb_max) %>%
  select(subject_code)

# plot just 7 of 32 that have failed concs
dailysis_with_failed_concs <- dailysis_with_failed_concs[1:7,]

dialysis_lp_plots(dailysis_with_failed_concs, "figures/dialysis_fixed_concentrations.pdf")
...
kable(recommendation_table.df[1:14], format = "latex", escape = F, booktabs = T, row.names = FALSE, align = c("r", rep("c",13)), 
   ...
   column_spec(2, color = recommendation_table.df[,15], bold = recommendation_table.df[,28], underline = recommendation_table.df[,15] == up_color) %>%
   column_spec(3, color = recommendation_table.df[,16], bold = recommendation_table.df[,29], underline = recommendation_table.df[,16] == up_color) %>%
   column_spec(4, color = recommendation_table.df[,17], bold = recommendation_table.df[,30], underline = recommendation_table.df[,17] == up_color) %>%
   column_spec(5, color = recommendation_table.df[,18], bold = recommendation_table.df[,31], underline = recommendation_table.df[,18] == up_color) %>%
   column_spec(6, color = recommendation_table.df[,19], bold = recommendation_table.df[,32], underline = recommendation_table.df[,19] == up_color) %>%
   column_spec(7, color = recommendation_table.df[,20], bold = recommendation_table.df[,33], underline = recommendation_table.df[,20] == up_color) %>%
   column_spec(8, color = recommendation_table.df[,21], bold = recommendation_table.df[,34], underline = recommendation_table.df[,21] == up_color) %>%
   column_spec(9, color = recommendation_table.df[,22], bold = recommendation_table.df[,35], underline = recommendation_table.df[,22] == up_color) %>%
   column_spec(10, color = recommendation_table.df[,23], bold = recommendation_table.df[,36], underline = recommendation_table.df[,23] == up_color) %>%
   column_spec(11, color = recommendation_table.df[,24], bold = recommendation_table.df[,37], underline = recommendation_table.df[,24] == up_color) %>%
   column_spec(12, color = recommendation_table.df[,25], bold = recommendation_table.df[,38], underline = recommendation_table.df[,25] == up_color) %>%
   column_spec(13, color = recommendation_table.df[,26], bold = recommendation_table.df[,39], underline = recommendation_table.df[,26] == up_color) %>%
   column_spec(14, color = recommendation_table.df[,27], bold = recommendation_table.df[,40], underline = recommendation_table.df[,27] == up_color) %>%
...
recdist <- ggplot(change_analysis.df, aes(y = nutrient_label, x = change)) +
  ...
  labs(y = "Nutrient", x = "Normalized change", title = "Distribution of recommended intake change over all renal patients") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) 

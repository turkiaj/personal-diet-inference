...
for (graph_person_id in 2:2) {
  ...
  if (check_failed_concs(graph_person_id, personal_info, recommeded_concentrations) == TRUE) { 
      ...
      for (target_index in 1:nrow(assumedtargets)) {
        ...
        temp.df$concentration <- target_name

        # Transform personal nutrient effects and their concentration contribution into original scale for reporting
        contributions_org <- c()
        ...
        for (nutrient_index in 1:length(queried_nodes)) {
          ...
          effect_name <- paste0("personal_", queried_nodes[nutrient_index], "_", target_name)
          
          personal_beta_norm <- V(personal_graph)[effect_name]$value
          personal_beta_norm_lCI <- V(personal_graph)[effect_name]$value_lCI
          personal_beta_norm_uCI <- V(personal_graph)[effect_name]$value_uCI

          personal_beta_org <- personal_beta_norm / org_stats$sd[nutrient_index]
          personal_beta_org_lCI <- personal_beta_norm_lCI / org_stats$sd[nutrient_index]
          personal_beta_org_uCI <- personal_beta_norm_uCI / org_stats$sd[nutrient_index]
  
          baseline_adj <- -(personal_beta_norm * org_stats$mean[nutrient_index]) / org_stats$sd[nutrient_index]
          baseline_adj_lCI <- -(personal_beta_norm_lCI * org_stats$mean[nutrient_index]) / org_stats$sd[nutrient_index]
          baseline_adj_uCI <- -(personal_beta_norm_uCI * org_stats$mean[nutrient_index]) / org_stats$sd[nutrient_index]
  
          Q_proposal <- result$Q[,nutrient_index] * org_stats[org_stats$name == nutrient_name,]$sd + org_stats[org_stats$name == nutrient_name,]$mean
        ...
        temp.df$contributions <- contributions_org
        temp.df$contributions_lCI <- contributions_org_lCI
        temp.df$contributions_uCI <- contributions_org_uCI
        
        baselevel <- head(result$Y_mu_Q0[,target_index],1)
        temp.df <- rbind(temp.df, c("base", target_name, baselevel, baselevel, baselevel))
...
for (subject_code in recommendation_with_concpreds.df$subject_code) {
  ...
  for (i in 1:length(queried_nodes))
  {
    ...
    for (t in dialysis_targets$Name)
    {
      ...
      # Graphs hold the normalized effects. Transform these too to original scale for reporting.
      effect_sd <- org_stats$sd[i]
      effects_org <-  c(graph_person_id,
                          effect_name,
                          nodes[paste0("personal_", effect_name)]$value / effect_sd,
                          nodes[paste0("personal_", effect_name)]$value_lCI / effect_sd,
                          nodes[paste0("personal_", effect_name)]$value_uCI / effect_sd)
...
kable(effect_summary_table.df[c(6,2:5)], "html", escape = F, booktabs = T, row.names = FALSE, digits = 2,align="lcccc",

...
```{r data_loading, echo=FALSE, message=FALSE, warning=FALSE}
<- dialysis_datadesc[dialysis_datadesc$Order==100,]
...
effect_summary_table.df[effect_summary_table.df$nutrient == "Phosp.",][c("effect_description", "min_personal", "max_personal")]
...
dialysis_lp_plots <- function(subject_code_vector, plot_path) {
  ...
  for (subject_code in subject_code_vector[1:(persons-1)]) {
      ...
      subsampled_intake.df <- remove_overlapping_points(accepted_intake.df, pk, lp, subsampling_tolerance)
      ...
      subsampled_intake.df <- remove_overlapping_points(accepted_intake.df, fppi, lp, subsampling_tolerance)
      ...
      subsampled_intake.df <- remove_overlapping_points(accepted_intake.df, palb, lp, subsampling_tolerance)
...
ggplot(dialysis, aes(x=sakkaroosiepros, y=palb, group = potilas)) +

...
pe_summary <- summary(target_blmm, pars="g_alpha")

eff1 <- na.omit(pe_summary1$summary[,9])
c(min(eff1),mean(eff1),max(eff1))

rhat1 <- na.omit(pe_summary1$summary[,10])
c(min(rhat1),mean(rhat1),max(rhat1))
...
dialysis_lp_plots <- function(subject_code_vector, plot_path) {
  ...
  for (subject_code in subject_code_vector[1:(persons-1)]) {
      ...
      dialysis[dialysis$potilas == subject_code,]
...
dialysis_with_failed_concs <- subjects_with_failed_concs[1:8,]
